...
function loadImg(thislink, thislinkencoded, thisname, thisID, ext){
    ...
    if (modalzoomview && vfmmodals.hasOwnProperty('zoomview')) {
        ...
        $(".vfm-zoom").html('<div class="prezoomimg"><div class="position-absolute w-100 h-100 start-0 top-0 d-flex justify-content-center align-items-center"><div class="spinner-border" role="status"></div></div></div><img class="preimg" src="admin/include/vfm-thumb.php?thumb='+ thislink +'&y=1"/>');
    ...
    $('.mailpreload').fadeIn(function(){
        $.ajax({
            ...
            .done(function( msg ) {
                ...
                $('.mailpreload').fadeOut();
                $('#captcha').attr('src', 'admin/captcha/img.php?' + time);
        ...
        if (!$(this).hasClass('disabled')) {
            ...
            if (search.length) {
                ...
                $.ajax({
                    ...
                    type: "GET",
                    dataType: 'json',
                    url: "admin/ajax/get-search.php?s=" + search
                })
                .done(function( msg ) {
...
function createZip(files, folder, dash, time = false, onetime = false){
    ...
    .done(function( msg ) {
        ...
        if (json.error == false) {
            var link = vfmvars.settings.prettylinks == true ? 'download/zip/'+json.json +'/n/'+json.supah : 'admin/vfm-downloader.php?zip='+json.json +'&n='+json.supah;
    ...
    if (vfmmodals.hasOwnProperty('foldertree')) {
        ...
        $('.archive-map').on('show.bs.modal', function (event) {
            ...
            if (!wrapresult.hasClass('loaded')) {
                ...
                var request = $.ajax({
                    url: "admin/ajax/get-filetree.php",
                    method: "POST",
                    data: { 
                        'action' : action,
    ...
    if (vfmmodals.hasOwnProperty('move')) {
        ...
        $(document).on('click', '.movelink', function(e) {
            ...
            $.ajax({
                cache: false,
                type: "POST",
                url: "admin/ajax/vfm-move.php?tm=" + (new Date()).getTime(),
                data: movedata
            })
            .done(function(msg) {
                if (msg == 'ok') {
...
if ($('body').data('ping')) {
    var audio_ping = new Audio("admin/"+$('body').data('ping'));
}
